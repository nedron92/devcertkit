...
[[ -n "$CLIENT_NAME" ]] || fail "Missing client name."
...
create_vpn_client() {
  ...
  cp -f "$VPN_TA_KEY" "$VPN_OUTPUT_DIR/$CLIENT_NAME/"

  local output_dir="${VPN_OUTPUT_DIR}/${CLIENT_NAME}"
  ...
  case "$TYPE" in
    general)
      cfg="${output_dir}/${CLIENT_NAME}-default.ovpn"
      cfg2="${output_dir}/${CLIENT_NAME}-fallback.ovpn"
      ...
      ;;
    mobile)
      cfg="${output_dir}/${CLIENT_NAME}.ovpn"
      cp "${TEMPLATE_DIR}/ovpn-mobile-client.ovpn" "$cfg"
      ;;
    router)
      cfg="${output_dir}/${CLIENT_NAME}.conf"
